...
/*
	...
	*/
	...
	for (int i = 0; i < enemigoEmmiter->GetVector().size();i++) 
	{
	    //bool resultado = enemigoEmmiter->GetVector()[i].CalcularCollision(balasEmmiter->GetVector());
		Solid* enemigo = enemigoEmmiter->GetVector()[i];
		int holaMario = enemigo->BalaColisionadora(balasEmmiter->GetVector());
		if (enemigo)
		{
			bool resultado = enemigo->CalcularCollision(balasEmmiter->GetVector());
			/*if (resultado) {
				cout << resultado << endl; 
				//aqui hay que actualizar la variable puntos
				enemigoEmmiter->BorrarParticula(i);
				balasEmmiter->BorrarParticula(enemigo->BalaColisionadora(balasEmmiter->GetVector()));
			}*/
			if (holaMario != -1) {
				enemigoEmmiter->BorrarParticula(i);
				//balasEmmiter->BorrarParticula(holaMario);
			}
		}
	}
